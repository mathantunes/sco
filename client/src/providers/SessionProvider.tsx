...
export function SessionProvider({
  ...
  async function addItem(productId: string, quantity = 1) {
    await run(async () => {
      const order = requireOpenOrder()
      const response = await request<{ order: OpenOrder }>(
        `/orders/${order.id}/items?deviceId=${encodeURIComponent(deviceId)}`,
        {
          method: 'POST',
  ...
  async function updateItemQuantity(itemId: string, quantity: number) {
    await run(async () => {
      const order = requireOpenOrder()
      const response = await request<{ order: OpenOrder }>(
        `/orders/${order.id}/items/${encodeURIComponent(itemId)}?deviceId=${encodeURIComponent(deviceId)}`,
        {
          method: 'PUT',
    ...
    try {
      await run(async () => {
        ...
        const response = await request<{ order: PaidOrder }>(
          `/orders/${order.id}/checkout?deviceId=${encodeURIComponent(deviceId)}`,
          { method: 'POST' },
        )
