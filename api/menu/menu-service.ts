...
export class MenuService {
    async getMenu(storeId: string): Promise<Menu | null> {
        return menus[storeId as keyof typeof menus] ?? null;
    }
    ...
    async getProductById(storeId: string, productId: string): Promise<Product | null> {
        ...
        const products = menu.categories.flatMap(cat => cat.items).map(item => item.product);
